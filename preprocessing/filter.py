    ...
    def transform(self, X):
        ...
        try:
            for row in range(X.shape[0]):
                coef = np.polynomial.polynomial.polyfit(self.Xref, X[row,:], deg=1)
                X[row,:] = (X[row,:] - coef[0])/coef[1]
        except Exception as e:
            raise ValueError(
                "Cannot perform MSC transformation : {}".format(
                    e
                )
            )
...
class SNV:
    ...
    def fit(self, X, y=None):
        """Stores the size of the data (number of columns) for consistency later."""
        X = check_array(X, accept_sparse=False)
        
        assert (0. < self.q) and (100. > self.q)
    ...
    def transform(self, X):
        ...
        try:
            ...
            raise ValueError(
                "Cannot perform SNV transformation : {}".format(
                    e
                )
            )
