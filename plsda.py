...
class PLSDA:
    def __init__(self, alpha=0.05, gamma=0.05, n_components=2, not_assigned=-1, style='soft'):
        """
        PLS-DA for classification. Implements 'hard' classification as an 'LDA-like' 
        criterion, and a 'soft' classification using a 'QDA-like' criterion as
        described in [1].  Soft PLS-DA may assign a point to 0, 1, or >1 classes,
        while the hard PLS-DA always assigns exactly one class to a point.
    ...
    def check_category_type_(self, y):
        """Check that categories are of the same type as 'not_assigned' variable."""
        t_ = None
        ...
        if not np.all([isinstance(y_, use_type) for y_ in y]):
            raise ValueError("You must set the 'not_assigned' variable type ({}) the same as y, e.g., \
            both must be int or str".format(use_type))
    ...
    def fit(self, X, y):
        ...
        y : array-like
            Ground truth classes - will be converted to numpy array automatically.
        ...
        self.__ohencoder_ = OneHotEncoder(sparse=False) # Convert integers to OHE
        self.__x_pls_scaler_ = StandardScaler(with_mean=True, with_std=True) # Center and scale X
        self.__y_pls_scaler_ = StandardScaler(with_mean=True, with_std=False) # Center do not scale Y
        
        self.__ohencoder_.fit(self.__y_)
        assert(self.not_assigned not in set(self.__ohencoder_.categories_[0])), 'not_assigned value is already taken'
        
        self.__class_mask_ = {}
        for i in range(len(self.__ohencoder_.categories_[0])):
            self.__class_mask_[i] = (self.__y_ == self.__ohencoder_.categories_[0][i]).ravel()
        
        self.__y_ = self.__y_pls_scaler_.fit_transform(self.__ohencoder_.transform(self.__y_))
        self.__X_ = self.__x_pls_scaler_.fit_transform(self.__X_) 
    
        # 2. PLS2
        self.__plsda_ = PLSRegression(n_components=self.n_components, 
                              max_iter=5000, 
                              tol=1.0e-9,
        ...
        self.__pca_ = PCA(n_components=len(self.__ohencoder_.categories_[0])-1, random_state=0)
        self.__T_train_ = self.__pca_.fit_transform(self.__y_pca_scaler_.fit_transform(y_hat_train))

        self.__class_centers_ = self.__pca_.transform(self.__y_pca_scaler_.transform(np.eye(len(self.__ohencoder_.categories_[0])))) 
        ...
        self.__S_ = {}
        for i in range(len(self.__ohencoder_.categories_[0])):
            self.__S_[i] = np.zeros((self.__T_train_.shape[1], self.__T_train_.shape[1]), dtype=np.float64)
            for t in self.__T_train_[self.__class_mask_[i]]:
                # Same as an outer product
                self.__S_[i] += (t - self.__class_centers_[i]).reshape(-1,1).dot((t - self.__class_centers_[i]).reshape(-1,1).T)
        ...
        # See Pomerantsev & Rodionova "multiclass pls tutorial" 2017
        L = np.cov(self.__T_train_.T)
        assert(np.allclose(L, np.eye(len(self.__pca_.explained_variance_))*self.__pca_.explained_variance_))
        self.__L_ = L
    
        # 5. Compute Mahalanobis critical distance
        self.__d_crit_ = scipy.stats.chi2.ppf(1.0-self.alpha, len(self.__ohencoder_.categories_[0])-1)
        self.__d_out_ = [scipy.stats.chi2.ppf((1.0-self.gamma)**(1.0/np.sum(self.__class_mask_[i])), len(self.__ohencoder_.categories_[0])-1) 
                 for i in range(len(self.__ohencoder_.categories_[0]))
    ...
    def check_outliers(self):
        ...
        -------
        outliers : ndarray
            Boolean mask of X_train used in fit() of if each point is considered an outlier.
        """"
        # We can only assess outliers on the training data
        # Others in test set will be "not assigned" and should be assumed correct - it is just the 
        # training stage where we can look at bad data.
        ...
        for j,t in enumerate(self.__T_train_):
            ...
            for i in range(len(self.__ohencoder_.categories_[0])): 
                if self.__class_mask_[i][j]:
                    cat = i
                    break
            d = np.matmul(np.matmul((t - self.__class_centers_[cat]), np.linalg.inv(self.__S_[cat])), 
                          (t - self.__class_centers_[cat]).reshape(-1,1))[0]
    ...
    def predict(self, X):
        ...
        predictions : list(list)
            ...
            predictions for each entry, and are listed fro left to right in order of decreasing
            likelihood.
        """
        ...
        for t in T_test:
            if self.style == 'soft': # This 'soft' rule is based on QDA
                distances.append([np.matmul(
                    np.matmul((t - self.__class_centers_[i]), np.linalg.inv(self.__S_[i])), 
                    (t - self.__class_centers_[i]).reshape(-1,1))[0] 
                          for i in range(len(self.__ohencoder_.categories_[0]))])
            else: # This 'hard' rule is based on LDA
                distances.append([np.matmul(
                    np.matmul((t - self.__class_centers_[i]), np.linalg.inv(self.__L_)), 
                    (t - self.__class_centers_[i]).reshape(-1,1))[0] 
                          for i in range(len(self.__ohencoder_.categories_[0]))])
        assert(np.all(np.array(distances)>=0)), 'All distances must be >= 0' 
        
        # Return all classes within d_crit, sorted from smallest to largest for soft version.
        # "NOT_ASSIGNED" means no assignment.
        predictions = []
        for row in distances:
            d = sorted(zip(self.__ohencoder_.categories_[0], row), key=lambda x:x[1]) # The lower d, the higher the certainty of that class
            if self.style == 'soft':
    ...
    def figures_of_merit(self, predictions, actual):
        ...
        predictions : list(list)
            ...
            multiple entries corresponding to multiple class predictions in the soft PLS-DA case.
        actual : array-like
        ...
        assert(self.not_assigned not in set(actual)), 'not_assigned value is already taken'
        
        all_classes = [self.not_assigned] + np.unique(np.concatenate((np.unique(actual), trained_classes))).tolist()
        ...
        df = df[df.index != self.not_assigned] # Trim off final row of "NOT_ASSIGNED" since these are real inputs
        ...
        # A very bad classifier could assign wrong classes often and with soft method TSPS < 0 is possible
        TSPS = np.max([0, TSPS])
        TEFF = np.sqrt(TSPS*TSNS)

        # Only return FoM for classes seen during training
        return (df[[c for c in df.columns if c != self.not_assigned]+[self.not_assigned]], # Re-order for easy visualization 
                I, 
                CSNS, 
                CSPS, 
                CEFF, 
    ...
    def score(self, X, y, use='TEFF'):
        ...
        y : array-like
            Ground truth classes - will be converted to numpy array automatically.
        use : str
            Which metric to use as the score.  Can be {TEFF, TSNS, TSPS} (default=TEFF).
        ...
        df, I, CSNS, CSPS, CEFF, TSNS, TSPS, TEFF = self.figures_of_merit(self.predict(X), y)
    ...
    def _get_tags(self):
        ...
        return {
            ...
            '_skip_test':True, # Skip for now since get_tags is unstable anyway
            '_xfail_checks':False,
            'stateless':False,
            'X_types':['2darray'],
        }
