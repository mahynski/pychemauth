...
def generate_response(X, dimension, mean=[0,0], seed=1, y_center=0, display=True):
...
def visualize_data(X, mean=[0,0]):
    ...
    for i, (comp, var) in enumerate(zip(pca.components_, pca.explained_variance_)):
        ...
        axes[0].plot([mean[0], comp[0]+mean[0]], [mean[1], comp[1]+mean[1]], label=f"Principal Component {i+1}", linewidth=3,
                 color=f"C{i + 2}")
    ...
    axes[1].plot(projection_0[:,0], projection_0[:,1], 'g*', alpha=0.1, label='Projection to PC 1')
    axes[1].plot(projection_1[:,0], projection_1[:,1], 'r*', alpha=0.1, label='Projection to PC 2')
