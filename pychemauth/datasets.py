...
def load_pgaa(return_X_y=False, as_frame=False):
    ...
    centers = pd.read_csv(io.StringIO(requests.get(url).content.decode('utf-8')), dtype=float)
    ...
    X = pd.read_csv(io.StringIO(requests.get(url).content.decode('utf-8')), dtype=float)
    ...
    y = pd.read_csv(io.StringIO(requests.get(url).content.decode('utf-8')), dtype=str).squeeze()
