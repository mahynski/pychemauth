...
class OpenSetClassifier(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        if (self.score_using not in self.knowns_) and (self.score_using.lower() != "all"):
            raise ValueError("score_using should be 'all' or one of the classes trained on.")
        ...
        try:
            ...
            raise Exception(f"Unable to fit outlier model : {sys.exc_info()[0]}")  
        ...
        if np.sum(composite_mask) == 0:
            raise Exception("There are no inlying known classes in the training set.")
            
        if len(np.unique(y[composite_mask])) < 2:
            raise Exception("There are less than 2 distinct classes available for training.")
        
        try:
            self.clf_ = self.clf_model(**self.clf_kwargs)
            self.clf_.fit(X[composite_mask, :], y[composite_mask])
        except:
            raise Exception(f"Unable to fit classification model : {sys.exc_info()[0]}")
    ...
    def figures_of_merit(self, predictions, actual):
        ...
        all_classes = [self.unknown_class] + np.unique(np.concatenate((np.unique(actual), self.knowns_))).tolist()
        ...
        for row, actual_class in zip(predictions, actual):
            ...
            if isinstance(row, np.ndarray) or isinstance(row, list):
                ...
                if self.clf_style.lower() == 'soft':
                    raise Exception("Class assignments not provided as list - perhaps you are using a hard model or OCC?")
        ...
        if self.score_using.lower() == "all":
            ...
            TSPS = 1.0 - (
                ...
                1.0 if self.clf_style.lower() == "hard" else len(self.knowns_) - 1.0
            )
            ...
            if np.all(actual == self.score_using):
                ...
                TSPS = 1.0 - (df[self.score_using].sum() - df[self.score_using][self.score_using]) / (Itot.sum() - Itot[self.score_using])
            ...
            if self.score_using not in set(actual):
                ...
                TSNS = df[self.score_using][self.score_using] / Itot[self.score_using]
