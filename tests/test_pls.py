...
class BenchmarkMdatools_ScaledRobust(unittest.TestCase):
    ...
    def test_distances(self):
        ...
        h = np.array([10.29014234,  5.68396294,  8.45943246,  4.65207861,  4.67722056,
            15.79446821,  6.62819327,  3.12982965,  7.07121626,  4.38433553,
            4.84622103,  6.59608612,  6.53752951,  7.9270221 ,  8.63861616,
            6.15785337,  7.54084761,  4.02231233,  5.05095549,  5.21724246,
            10.4166253 ,  5.26796247,  6.27498745,  5.73485876])
        q = np.array([0.43389152, 0.14470249, 0.26963732, 0.00874239, 0.04608073,
            0.02633357, 0.03248984, 0.03455517, 0.16067683, 0.26912052,
            0.22612725, 0.04602259, 0.02272952, 0.05832545, 0.37249989,
            0.05211344, 0.19858228, 0.0941601 , 0.10476625, 0.01081812,
            0.000926  , 0.22716434, 0.10918501, 0.65167524])
        ...
        z = np.array([1.58228921e-01, 3.94751369e-02, 3.77222896e-02, 3.30816568e-01,
            1.85407827e-02, 5.24226344e-02, 1.27932484e-01, 4.12414812e-02,
            6.06064957e-01, 3.88376416e-01, 1.39654903e-01, 8.83245922e-02,
            1.39894300e-01, 5.92985925e-03, 3.24497189e-03, 5.64916449e-02,
            4.13179574e-02, 5.40697478e-04, 1.47805513e-01, 5.04271503e-01,
            2.14004278e-04, 5.00918846e-02, 1.52019455e+00, 4.31411532e-02])

        np.testing.assert_almost_equal(self.model._h_q(self.Xc)[0], h, decimal=6)
        np.testing.assert_almost_equal(self.model._h_q(self.Xc)[1], q, decimal=6)
        np.testing.assert_almost_equal(self.model._z(self.Xc, self.yc), z, decimal=6)

    def test_limits(self):
        """Test h, q, z, f limits."""
        np.testing.assert_almost_equal(6.433454627069881, self.model._PLS__h0_, decimal=6)
        ...
        np.testing.assert_almost_equal(0.15928871670572453, self.model._PLS__q0_, decimal=6)
        np.testing.assert_almost_equal(0.10543040976824493, self.model._PLS__z0_, decimal=6)
        ...
        np.testing.assert_equal(self.model._PLS__Nf_, self.model._PLS__Nh_ + self.model._PLS__Nq_)

    def test_predictions(self):
        """Test predicted y values."""
        preds = np.array([[35.63128259],
        [42.2737451 ],
        [33.52252497],
        [43.01107763],
        [37.08153933],
        ...
        [41.66132062],
        [35.8585947 ]])
        np.testing.assert_almost_equal(preds, self.model.predict(self.Xt), decimal=6)

    def test_score(self):
        """Test R^2 calculation."""
        np.testing.assert_almost_equal(0.9873776585483119, self.model.score(self.Xc, self.yc), decimal=6)
        np.testing.assert_almost_equal(0.9051699790584818, self.model.score(self.Xt, self.yt), decimal=6)

    def test_categorize(self):
        """Test extreme/outlier detection."""
        ext = np.array([True, False, False, False, False,  True, False, False, False,
            False, False, False, False, False, False, False, False, False,
            False, False, False, False, False, False])
        out = np.array([False, False, False, False, False, False, False, False, False,
            False, False, False, False, False, False, False, False, False,
            False, False, False, False, False, False])
        np.testing.assert_equal(ext, self.model.check_xy_outliers(self.Xc, self.yc)[0])
        np.testing.assert_equal(out, self.model.check_xy_outliers(self.Xc, self.yc)[1])

        ext = np.array([False,  True, False, False, False, False, False, False])
        out = np.array([True, False,  True, False,  True, False,  True, False])

        np.testing.assert_equal(ext, self.model.check_xy_outliers(self.Xt, self.yt)[0])
        np.testing.assert_equal(out, self.model.check_xy_outliers(self.Xt, self.yt)[1])
...
class BenchmarkMdatools_Scaled(unittest.TestCase):
    ...
    def test_limits(self):
        """Test h, q, z, f limits."""
        np.testing.assert_almost_equal(6.708333333333333, self.model._PLS__h0_, decimal=6)
        np.testing.assert_almost_equal(0.15005524333227455, self.model._PLS__q0_, decimal=6)
        np.testing.assert_almost_equal(0.1892474666263504, self.model._PLS__z0_, decimal=6)
...
class BenchmarkMdatools_UnscaledRobust(unittest.TestCase):
    ...
    def test_distances(self):
        """Test h, q, z distances."""
        h = np.array([10.16012747,  7.47244224,  5.53044946,  4.13920174,  2.8048815 ,
            12.86596107,  5.87311824,  3.31835631,  7.94855802,  6.07635506,
            7.19903601,  4.86277004,  6.50457825,  7.86213481,  9.3495848 ,
            5.49094806,  1.81682582,  5.37668895,  5.3674368 ,  5.02186922,
            8.85073553,  5.38273693,  8.87643819, 12.84876547])
        q = np.array([14.04170827,  0.17045717,  0.17624006,  0.15934482,  1.94461148,
            6.33353444,  9.10658786,  3.79674233,  0.14996447,  0.37547292,
            1.41237012, 15.97249387,  0.10225634,  4.99075633, 12.69703894,
            7.36851262, 18.80177517,  0.31126861,  0.75446964,  0.16304307,
            4.5955245 ,  4.32964194,  0.11064471, 27.26946415])
        z = np.array([0.03041174, 0.07500194, 0.26865468, 0.41675539, 0.07220708,
            0.14245054, 0.07646303, 0.10221563, 0.59254802, 0.56562692,
            0.14034235, 0.12690661, 0.22535524, 0.00598724, 0.03662903,
            0.071516  , 0.14186018, 0.05203803, 0.05441322, 0.50914342,
            0.00750257, 0.03739828, 1.72671652, 0.0038618 ])
    ...
    def test_limits(self):
        """Test h, q, z, f limits."""
        np.testing.assert_almost_equal(6.220076403682505, self.model._PLS__h0_, decimal=6)
        np.testing.assert_almost_equal(6.276821593745483, self.model._PLS__q0_, decimal=6)
        np.testing.assert_almost_equal(0.17504517218907578, self.model._PLS__z0_, decimal=6)
    ...
    def test_score(self):
        """Test R^2 calculation."""
        np.testing.assert_almost_equal(0.9847652, self.model.score(self.Xc, self.yc), decimal=6)
        np.testing.assert_almost_equal(0.9141127, self.model.score(self.Xt, self.yt), decimal=6)

    def test_categorize(self):
        """Test extreme/outlier detection."""
        ext = np.array([False, False, False, False, False,  True, False, False, False,
            False, False, False, False, False, False, False, False, False,
...
class BenchmarkMdatools_Unscaled(unittest.TestCase):
    ...
    def test_limits(self):
        """Test h, q, z, f limits."""
        np.testing.assert_almost_equal(6.708333333333333, self.model._PLS__h0_, decimal=6)
        np.testing.assert_almost_equal(5.630580159752494, self.model._PLS__q0_, decimal=6)
        np.testing.assert_almost_equal(0.22841689421337583, self.model._PLS__z0_, decimal=6)
...
class TestPLS_Scaled(unittest.TestCase):
    ...
    def test_h_q(self):
        ...
        ans_h = np.array([0.08272128, 0.12718815, 0.0304968]) * (self.X.shape[0]-1)
...
class TestPLS_Unscaled(unittest.TestCase):
    ...
    def test_h_q(self):
        """Check some h and q values."""
        h, q = self.model._h_q(self.X)
        ans_h = np.array([0.13355759, 0.13201587, 0.03812691]) * (self.X.shape[0]-1)
