...
class CheckDatasetWriting(unittest.TestCase):
    ...
    def test_fresh(self):
        ...
        try:
            x_f_, y_ = write_dataset(dir_.name+'/dummy', self.X, self.y, fmt='npy', overwrite=False, augment=False)
    ...
    def test_overwrite_catch(self):
        """Test overwrite caught."""
        dir_ = tempfile.TemporaryDirectory()
        
        with pytest.raises(
            Exception,
            match=f"{dir_.name}/dummy already exists."
        ):
            _ = write_dataset(dir_.name+'/dummy', self.X, self.y, fmt='npy', overwrite=False, augment=False)
            _ = write_dataset(dir_.name+'/dummy', self.X, self.y, fmt='npy', overwrite=False, augment=False)
    ...
    def test_overwrite_success(self):
        ...
        try:
            _ = write_dataset(dir_.name+'/dummy', self.X, self.y, fmt='npy', overwrite=False, augment=False)
            _ = write_dataset(dir_.name+'/dummy', self.X, self.y, fmt='npy', overwrite=True, augment=False)
        except Exception as e:
    ...
    def test_augment(self):
        ...
        try:
            x_f1_, y1_ = write_dataset(dir_.name+'/dummy', self.X, self.y, fmt='npy', overwrite=False, augment=False)
            _, idx1 = _sort_xdata(dir_.name+'/dummy')
            x_f2_, y2_ = write_dataset(dir_.name+'/dummy', self.X, self.y, fmt='npy', overwrite=False, augment=True)
        ...
        for a_, b_ in zip(x_f1_, x_f2_):
            self.assertEqual(
                ...
                int(b_.split('x_')[1].split('.npy')[0])-325 # Check that these are offset by 325
            )
