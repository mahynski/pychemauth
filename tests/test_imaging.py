...
class CheckCAMArchitecture(unittest.TestCase):
    ...
    def test_hires(self):
        ...
        def _renumber(answer, incr):
            ...
            result = '_'.join(prefix+[str(incr)]) #int(answer[3].split('_')[-1]) + 
        ...
        for model_idx in sorted(self.correct_answers.keys()):
            res = explain._check_cam_model_architecture(
                model_factory(model_idx), 
            ...
            correct = _renumber(self.correct_answers[model_idx], base+(model_idx-start)*incr) if model_idx <= 35 else self.correct_answers[model_idx]
            np.testing.assert_equal(res, correct)

    def test_grad(self):
        """Test the models for GradCAM."""
        for model_idx in self.correct_answers.keys():
            if model_idx in [0, 1, 2, 3, 12, 13, 14, 15, 20, 21, 22, 23, 28, 29, 30, 31]: # Non-CAM architectures should not allow GradCAM
                with pytest.raises(Exception, match="Cannot safely explain this model with GradCAM; use HiResCAM instead."):
                    explain._check_cam_model_architecture(
                        model_factory(model_idx), 
                        conv_layer_name=None, 
                        mode=None, 
                        style='grad'
