...
class TestOSC_HardMulticlass(unittest.TestCase):
    ...
    def test_isolationforest(self):
        ...
        np.testing.assert_almost_equal(osc.score(X_train_, y_train_), 0.9219544457292888, decimal=9)

        np.testing.assert_equal(
            osc.figures_of_merit(osc.predict(X_train_), y_train_)['CM'].values.ravel(),
            [35, 0, 5, 0, 33, 7], 
        )

        np.testing.assert_equal(
            osc.figures_of_merit(osc.predict(self.X_train), self.y_train)['CM'].values.ravel(),
    ...
    def test_elliptical(self):
        ...
        np.testing.assert_almost_equal(osc.score(X_train_, y_train_), 0.8874119674649424, decimal=9)
...
class TestOSC_SoftMulticlass(unittest.TestCase):
    ...
    def test_isolationforest(self):
        ...
        np.testing.assert_almost_equal(osc.score(X_train_, y_train_), 0.9013878188659973, decimal=9)
...
class TestOSC_BinaryOvA_Class(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.mask = (self.y_train == target) | (self.y_train == known_alternative)
    ...
    def test_isolationforest(self):
        ...
        np.testing.assert_almost_equal(osc.score(self.X_train[self.mask], self.y_train[self.mask]), 0.9219544457292888, decimal=9)

        np.testing.assert_equal(
            osc.figures_of_merit(osc.predict(self.X_train[self.mask]), self.y_train[self.mask])['CM'].values.ravel(),
            [35, 0, 5, 0, 33, 7], 
        )
...
class TestOSC_BinaryOvA_OCC(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        known_alternative = 'virginica'
        unknown_alternative = 'versicolor'
        self.mask = (self.y_train == self.target) | (self.y_train == known_alternative)
        ...
        self.y_train_binary = convert_y_to_binary_format(self.y_train[self.mask], self.target)
    ...
    def test_isolationforest(self):
        ...
        np.testing.assert_almost_equal(osc.score(self.X_train, self.y_train), 0.9354143466934853, decimal=9)
